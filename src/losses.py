...
class DeepFLAIRLoss(nn.Module):
    def __init__(self, mse_weight=1.0, ssim_weight=1.0, grad_weight=1.0):
        super().__init__()
        # 1. Switched to L1 Loss for better signal in dark regions
        self.l1 = nn.L1Loss()
    ...
    def forward(self, y_pred, y_true):
        ...
        total_loss = (self.w_l1 * l_l1) + (self.w_ssim * l_ssim) + (self.w_grad * l_grad)
        
        return total_loss, {"l1": l_l1, "ssim_loss": l_ssim, "grad_loss": l_grad}
